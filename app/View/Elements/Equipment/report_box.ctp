				...
				<div class="problem_history_description">
					<div class="top_problem_history_description">
                        <span id="problem_title">
							<?php echo __("Game Report") ?> <?php echo $this->Utils->getTitleByDateRange($this->data['Problem']['from'], $this->data['Problem']['to']) ?>
		...
		<tr>
			<td class="td_description" valign="top">
				<div id="problem_description">
					<div style="float:left; margin: 10px 30px 0 30px;">
                      <h2><?php echo $one['Game']['name'] ?> - <?php echo $one[$model]['mfg_serial'] ?></h2>
                    </div>
                    <div style="float:right; margin: 0 30px;">
                      <p style="font-size:110%">
                      <?php
                      echo $this->Html->tag('span', __("Total Downtime:"), array('style' => 'font-weight:bold;'))." ".$this->Utils->seconds_to_string($totals['totals']['total'])."<br />";
                      $array_ids = out_of_order_ids();
					  foreach($array_ids as $id => $name):
						  $text = isset($totals['totals'][$id]) ?  $this->Utils->seconds_to_string($totals['totals'][$id]) : "0";
						  echo $this->Html->tag('span', __("Total ".$name.":"), array('style' => 'font-weight:bold;'))." ".$text."<br />";
					  endforeach;
                      
					  echo $this->Html->tag('span', __("Total in Parts:"), array('style' => 'font-weight:bold;'))." ".$this->Number->currency($total_parts['total'], 'USD');
                      ?><br />
                      </p>
                    </div>
                  </div>
			...
			<td id="td_problem_overview" valign="top" class="td_overview">
               <div class="div_overview">
				<p>
                    <?php echo $this->Html->tag('span', __("Game:"), array('style' => 'font-weight:bold;'))." ".$one['Game']['name'] ?><br />
                    <?php echo $this->Html->tag('span', __("Location:"), array('style' => 'font-weight:bold;'))." ".$one['Location']['name']; ?><br />
                    <?php echo $this->Html->tag('span', __("Route:"), array('style' => 'font-weight:bold;'))." ".$one['Location']['Route']['name']; ?><br />
                    <?php echo $this->Html->tag('span', __("Equiment Type:"), array('style' => 'font-weight:bold;'))." ".$one['EquipmentType']['name']; ?><br />
                    <?php echo $this->Html->tag('span', __("Mfg Serial:"), array('style' => 'font-weight:bold;'))." ".$one['Equipment']['mfg_serial']; ?>
				</p>
             </div>
			</td>
		</tr>
