...
var fmLib =  {
	...
	call: function(url, frmName) {
		...
		if(url.match('fmMode=edit')) {
			ajaxObj.makeRequest(url, function() {
				var contObj = document.getElementById('fmEditorCont');
	...
	viewResponse: function(json, curCont, listLoad) {
		var fmCont = document.getElementById(curCont);
		var titleCont = document.getElementById(curCont + 'Title');
		var expCont = document.getElementById(curCont + 'Exp');
		var listCont = document.getElementById(curCont + 'List');
		...
		if(typeof json == 'object') {
			...
			if(expCont && !json.login) this.getExplorer(json.cont);
	...
	initFileManager: function(url, mode) {
		...
		if(!fmContSettings[curCont]) {
			ajaxObj.makeRequest(url + '&fmMode=getContSettings', function() {
				...
				fmLib.setFileManagerSize(curCont);
					...
					ajaxObj.makeRequest(url + '&fmMode=getUserPerms', function() {
						...
						if(!fmMsg[fmContSettings[curCont].language]) {
							...
							ajaxObj.makeRequest(url + '&fmMode=getMessages', function() {
								...
								var fmCont = document.getElementById(curCont);
								var expCont = document.getElementById(curCont + 'Exp');
								var pw = (fmCont.style.width.indexOf('%') != -1);
								var ph = (fmCont.style.height.indexOf('%') != -1);
								var epw = (expCont && expCont.style.width.indexOf('%') != -1);
	...
	setFileManagerSize: function(curCont) {
		...
		if(typeof fmContSettings[curCont].fmWidth == 'string' && fmContSettings[curCont].fmWidth.indexOf('%') != -1) {
			width = Math.round(fmTools.getWindowWidth() * parseInt(fmContSettings[curCont].fmWidth) / 100) - 90;
		...
		if(document.getElementById(curCont + 'Title')) height -= 25;
		...
		if(expCont) {
			...
			else expCont.style.width = expCont.offsetWidth + 'px';
		}
			...
			default:
				...
				if(fmEntries[curCont][id].deleted) {
					...
					if(fmEntries[curCont][id].icon.substr(0, 4) == 'text') {
						if(userPerms.edit) {
							items.push({icon:'edit.gif',caption:fmMsg[lang].cmdEdit,dialog:'fmEditor'});
						}
						else if(!userPerms.hideDisabledIcons) {
