...
var fmParser = {
	...
	parseTitle: function(json, contObj) {
		...
		var width = document.getElementById(json.cont).offsetWidth - 110;
		...
		html += '<tr>\n';
		html += '<td class="fmTH1" align="left">\n';
		html += '<table border="0" cellspacing="0" cellpadding="0" width="100%">\n';
		html += '<tr>\n';
		html += '<td><div class="fmTH1" style="width:' + width + 'px; padding:4px; white-space:nowrap; overflow:hidden" title="' + title + '">' + title + '</div></td>\n';
	...
	parseMain: function(json, contObj) {
		...
		if(!json.login) {
			...
			var expCont = document.getElementById(json.cont + 'Exp');
			if(expCont) fmTools.touchScroll(expCont);
		}
		return true;
	},
	...
	parseEntriesDetailView: function(json, date) {
		...
		var listCont = document.getElementById(json.cont + 'List');
		...
		for(i in entries.captions) {
			...
			html += '<div class="fmTH6" style="' + style + '" title="' + tooltip + '" onMouseOver="this.className=\'fmTH4\'" onMouseOut="this.className=\'fmTH6\'" onMouseDown="this.className=\'fmTH5\'" onMouseUp="this.className=\'fmTH4\'" onClick="' + action + '">\n'; // Dario
		...
		html += '</div>\n';
		html += '<div id="' + json.cont + '_detBody" style="width:' + entries.width + 'px; height:' + (theight - 20) + 'px; overflow:auto">\n';
	...
	parseEntriesIconView: function(json, date) {
		...
		var cellCnt = 0;
		var action2 = '';
		var listWidth = document.getElementById(json.cont + 'List').offsetWidth;
		...
		for(i in entries.items) {
			...
			if(!entries.items[i].deleted && userPerms.mediaPlayer && entries.items[i].name.match(/\.(mp3|m4a|aac|wav|og[ga])$/i)) {
 			...
 			html += '<div class="' + cssData + '" style="overflow:hidden; white-space:nowrap; vertical-align:middle; width:' + (cellWidth - 2) + 'px" title="' + entries.items[i].name + '">' + name + '</div>\n';
 			html += '</td>\n';
			html += '</tr></table>\n';
 			html += '</td>\n';
			cellCnt++;
	...
	parseExplorer: function(json, link) {
		if(typeof json != 'object') return '';
		if(!document.getElementById(json.cont + 'Exp')) return '';
	...
	parseEditor: function(json, contObj) {
		...
		var btn = document.getElementById('fmEditorButton');
		var lang = fmContSettings[json.cont].language;

		if(btn && btn.innerHTML == '') {
			btn.innerHTML = '<img src="' + fmWebPath + '/icn/save.gif" width="14" height="14" style="cursor:pointer" title="' + fmMsg[lang].cmdSave + '" onClick="fmLib.callOK(\'' + fmMsg[lang].msgSaveFile + '\', \'\', \'frmEdit\')"/>';
		}
	...
	parseTitleIcons: function(json) {
		...
		if(fmContSettings[json.cont].listType == 'details') {
			icon = 'list_icons.gif'
			tooltip = fmMsg[lang].cmdIcons;
		}
		else {
			icon = 'list_details.gif'
			tooltip = fmMsg[lang].cmdDetails;
		}
