...
include_once('extra_functions.php'); //hanmuja
...
/* PUBLIC METHODS ****************************************************************************** */
	...
	function rename($dst) {
		/**
		 * Rename in database arcade
		 * Modified by hanmuja
		 */
		$ok = false;
		if($this->Listing->FileSystem->rename($this->path, $dst))
		{
			$extra_funcitons = new ExtraFunctions();
			$extra_funcitons->renameDb($this->path, $dst);
			$ok = true;
		}
		return $ok;
	}
