...
/* PUBLIC METHODS ****************************************************************************** */
	 ...
	 * @param string $engine		optional: engine - hanmuja
	 * @return FileManager
	 */
	function FileManager($startDir = '', $engine = 'Java', $user_id = 0) {
		$this->_incPath = str_replace('\\', '/', realpath(dirname(__FILE__) . '/..'));
		$this->_tmpDir = $this->_incPath . '/tmp';
		$this->initFromConfig($engine, $user_id);
	 ...
	 * @param string $engine		- hanmuja
	 */
	function initFromConfig($engine, $user_id) {
		$config = parse_ini_file($this->_incPath . '/config.inc.php');
		
		$config['uploadEngine'] = $engine;  //se agrega engine por seleccion - hanmuja
		
		/**
		 * Sacar fmWebPath de bd - hanmuja
		 */
		include('acl_manager.php');
		//include('db.php');
		$result = mysql_query("SELECT * FROM settings WHERE name = 'webroot'");
		$row = mysql_fetch_array($result);
		
		$config['fmWebPath'] = "http://".$row['val']."/tools/filemanager";
		$config['domain'] = "http://".$row['val'];
		
		//mysql_close($con);
			
		/**
		 * Permisos para filemanager
		 * hanmuja
		 */
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'create_folder'), $user_id, 'User')) $config['enableNewDir'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'rename_folder'), $user_id, 'User')) $config['enableRename'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'remove_folder'), $user_id, 'User')) $config['enableDelete'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'can_search'), $user_id, 'User')) $config['enableSearch'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'can_upload'), $user_id, 'User')) $config['enableUpload'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'can_download'), $user_id, 'User')) $config['enableDownload'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'can_copy'), $user_id, 'User')) $config['enableCopy'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'can_move'), $user_id, 'User')) $config['enableMove'] = 1;
		if (getPermission(array('plugin'=>null, 'controller'=>'Folders', 'action'=>'can_bulk_download'), $user_id, 'User')) $config['enableBulkDownload'] = 1;
	...
	function create($user) {
		...
		/* ugly workaround for FM_Tools::getMsg */  // --------- DARIO ----------- //
		...
		$this->_checkStartDir();
		//$this->_checkAccess($_COOKIE[$this->container . 'LoginPwd']); // no check pass -DARIO
		$this->_save();
...
/* PRIVATE METHODS ***************************************************************************** */
	...
	function _viewHeader() {
		...
		if(!$this->hideTitleBar) print "<div id=\"$this->_titleCont\" class=\"fmTH1\" style=\"height:25px\"></div>\n";
		print "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\"><tr>\n";
		if($this->explorerWidth) print "<td width=\"$expWidth\"><div id=\"$this->_expCont\" class=\"fmTREEbg\" style=\"overflow:auto\"></div></td>\n";
	...
	function _viewFooter() {
		...
		if($this->debugInfo) {
			$fmWidth = strstr($this->fmWidth, '%') ? $this->fmWidth : (int) $this->fmWidth . 'px';
			print "<div class=\"fmTH1\" style=\"width:$fmWidth; height:150px; margin-top:1px; padding:1px\">";
			print "<div id=\"$this->_infoCont\" class=\"fmTD2\" ";
			print "style=\"height:148px; text-align:left; overflow:auto\">\n";
			print "</div></div>\n";
		}
