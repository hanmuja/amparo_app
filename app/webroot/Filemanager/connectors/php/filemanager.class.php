...
class Filemanager {
  ...
  public function rename() {
	...
	/**
	 * Add for delete dot (.) at end of foldername
	 * @hanmuja
	 */
	if(is_dir($this->doc_root.$this->get['old'])) {
		$this->get['new'] = str_replace('.', '', $this->get['new']);
	}
  ...
  /**
   * Function to make changes in database arcade for folders permissions
   * @hanmuja
   */
  private function renameDb($array)
  {
	...
	mysql_query("UPDATE folders SET folder = REPLACE(folder, '".$array['Old Name']."', '".$array['New Name']."')");
	
	mysql_query("UPDATE default_folders SET folder = REPLACE(folder, '".$array['Old Name']."', '".$array['New Name']."')");
		
	mysql_close($con);
  }
  
  private function deleteDb($array)
  {
  	$x = explode('/', $array['Path']);
	$path = $x[(count($x) - 2)];
